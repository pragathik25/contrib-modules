      ...
      if ($key == 'tags') {
        ...
        if ($value) {
          foreach ($value as $tag) {
            if (preg_match("/StdVol/i", $tag->getName()) == 1 && stripos($tag->getName(), 'StdVol') == 0) {
              $term_name = str_replace('StdVol-', '', $tag->getName());
            ...
            if (preg_match("/StdTpc/i", $tag->getName()) == 1 && stripos($tag->getName(), 'StdTpc') == 0) {
              $term_name = str_replace('StdTpc-', '', $tag->getName());
              $term = \Drupal::entityTypeManager()
                ->getStorage('taxonomy_term')
            ...
            if (preg_match("/ElnTpc/i", $tag->getName()) == 1 && stripos($tag->getName(), 'ElnTpc') == 0) {
              $term_name = str_replace('ElnTpc-', '', $tag->getName());
              $term = \Drupal::entityTypeManager()
                ->getStorage('taxonomy_term')
                ->loadByProperties(['name' => $term_name, 'vid' => 'shopify_topics']);
              if ($term) {
            ...
            if (preg_match("/Lang/i", $tag->getName()) == 1 && stripos($tag->getName(), 'Lang') == 0) {
              $term_name = str_replace('Lang-', '', $tag->getName());
              $term = \Drupal::entityTypeManager()
                ->getStorage('taxonomy_term')
            ...
            if (preg_match("/StdPbc/i", $tag->getName()) == 1 && stripos($tag->getName(), 'StdPbc') == 0) {
              $term_name = str_replace('StdPbc-', '', $tag->getName());
              $term = \Drupal::entityTypeManager()
                ->getStorage('taxonomy_term')
          ...
          if ($vol_tid) {
            $this->set('field_volumes', $vol_tid);
          }
          if ($topic_tid) {
            $this->set('field_topics', $topic_tid);
          }
          if ($lang_tid) {
            $this->set('field_languages', $lang_tid);
          }
          if ($pub_tid) {
            $this->set('field_publishing_cycle', $pub_tid);
          }
        } else {
          $this->set('field_languages', '');
          $this->set('field_volumes', '');
          $this->set('field_languages', '');
          $this->set('field_publishing_cycle', '');
        }
      }
      if (isset($values['member_price'])) {
        $this->set('field_member_price', $values['member_price']);
      }
      else {
        $this->set('field-member_price', '');
      }
      if (isset($values['non_member_price'])) {
        $this->set('field_non_member_price', $values['non_member_price']);
      }
      else {
        $this->set('field_non_member_price', '');
      }
    }
