...
class LinkCheckerLinkExtractionTest extends BrowserTestBase {
   ...
   * {@inheritdoc}
   */
  public static $modules = ['filter', 'linkchecker', 'node', 'path'];
  ...
  protected function setUp() {
    ...
    $this->config('linkchecker.settings')
      ...
      ->save();
    $this->config('linkchecker.settings')
      ->set('base_path', 'unexistingdomain.org/')
      ->save();
...
EOT;
      ...
      if ($link) {
        $this->assertIdentical(
              ...
              new FormattableMarkup(
                  'Absolute URL %org_url matches expected result %check_url.',
        ...
        $this->fail(
              new FormattableMarkup('URL %check_url not found.', [
                '%check_url' => $check_url,
              ])
                );
      }
    ...
    $urls_in_database = \Drupal::entityQuery('linkcheckerlink')
      ->accessCheck()
      ->count()
      ->execute();
    ...
    $this->assertEqual(
          ...
          $urls_expected_count,
          new FormattableMarkup(
              'Found @urls_in_database URLs in database matches expected result of @urls_expected_count.',
              [
    ...
    // $this->assertRaw($this->t('@type %title has been updated.',
    // ['@type' => 'Basic page', '%title' => $edit["title[0][value]"]]));
    $this->assertText(
      $this->t('@type @title has been updated.', [
        '@type' => 'Basic page',
        '@title' => $edit['title[0][value]'],
      ])
    );
    ...
    // $this->assertIdentical($node->path, '/' .
    // $edit[0]['path'], format_string('URL alias
    // "@node-path" matches path "@edit-path".
    // ', array('@node-path' => $node->path,
    // '@edit-path' => $edit[0]['path'])));
    // Verify if the content links are extracted properly.
    $base_path =
            $this->config('linkchecker.settings')->get('default_url_scheme') .
            $this->config('linkchecker.settings')->get('base_path');
    ...
    $urls_relative = [
      ...
      './test/foo bar/is_valid-hack.test' =>
    ...
    foreach ($urls_relative as $org_url => $check_url) {
      ...
      if ($link) {
        ...
        // $check_url, new FormattableMarkup('Relative URL %org_url
        // matches expected result %check_url.',
        // ['%org_url' => $org_url, '%check_url' => $check_url]));
      }
      else {
    ...
    foreach ($urls as $url) {
      ...
      // @FIXME
      // $this->assertTrue($link->status, format_string
      // ('Link check for %url is enabled.', ['%url' => $url]));
    }
    ...
    foreach ($documentation_urls as $documentation_url) {
      ...
      // @FIXME
      // $this->assertFalse($link->status, format_string
      // ('Link check for %url is disabled.', ['%url' => $documentation_url]));
    }
  }
