...
class LinkCheckerAdminSettingsForm extends ConfigFormBase {
  ...
  public function __construct(
    ...
    FilterPluginManager $plugin_manager_filter,
    LinkCheckerService $linkchecker_checker,
    LinkExtractorBatch $extractorBatch,
    LinkCleanUp $linkCleanUp,
    UserStorageInterface $user_storage,
    LinkCheckerResponseCodesInterface $linkCheckerResponseCodes,
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['general'] = [
      ...
      '#description' => $this->t(
        'Configure the fields under each <a href=":url">content type</a> that should be scanned for broken links.',
        [
    ...
    $form['general']['linkchecker_check_links_types'] = [
      ...
      '#description' => $this->t(
        'A full qualified link (https://example.com/foo/bar) to a page is considered external, whereas an absolute (/foo/bar) or relative link (node/123) without a domain is considered internal.'
      ),
    ...
    $form['tag']['linkchecker_extract_from_object'] = [
      ...
      '#type' => 'checkbox',
      '#title' => $this->t(
              'Extract links in <code>&lt;object&gt;</code> and <code>&lt;param&gt;</code> tags'
      ),
      '#description' => $this->t(
              'Enable this checkbox if multimedia and other links in object and their param tags should be extracted. The object tag is used for flash, java, quicktime and other applets.'
      ),
    ];
    $form['tag']['linkchecker_extract_from_video'] = [
      ...
      '#type' => 'checkbox',
      '#title' => $this->t(
              'Extract links in <code>&lt;video&gt;</code> tags including their <code>&lt;source&gt;</code> and <code>&lt;track&gt;</code> tags'
      ),
      '#description' => $this->t(
              'Enable this checkbox if links in video tags should be extracted. The video element is used to embed video content.'
      ),
    ];
    ...
    $form['check']['linkchecker_check_library'] = [
      ...
      '#description' => $this->t(
              'Defines the library that is used for checking links.'
      ),
    ...
    $form['check']['linkchecker_check_connections_max'] = [
      ...
      '#description' => $this->t(
              'Defines the maximum number of simultaneous connections that can be opened by the server. Make sure that a single domain is not overloaded beyond RFC limits. For small hosting plans with very limited CPU and RAM it may be required to reduce the default limit.'
      ),
    ...
    $form['check']['linkchecker_check_useragent'] = [
      ...
      '#description' => $this->t(
              'Defines the user agent that will be used for checking links on remote sites. If someone blocks the standard Drupal user agent you can try with a more common browser.'
      ),
      '#default_value' => $config->get('check.useragent'),
      '#options' => [
        'Drupal (+https://drupal.org/)' => $this->t('Drupal (+https://drupal.org/)'),
        'Mozilla/5.0 (Windows NT 6.3; WOW64; Trident/7.0; Touch; rv:11.0) like Gecko' => $this->t(
                  'Windows 8.1 (x64), Internet Explorer 11.0'
        ),
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/46.0.2486.0 Safari/537.36 Edge/13.10586' => $this->t(
                  'Windows 10 (x64), Edge'
        ),
        'Mozilla/5.0 (Windows NT 6.3; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0' => $this->t(
                  'Windows 8.1 (x64), Mozilla Firefox 47.0'
        ),
        'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0' => $this->t(
                  'Windows 10 (x64), Mozilla Firefox 47.0'
        ),
      ],
    ];
    $intervals = [
    ...
    $form['check']['linkchecker_check_interval'] = [
      ...
      '#description' => $this->t(
              'This interval setting defines how often cron will re-check the status of links.'
      ),
    ...
    if (
          $this->moduleHandler->moduleExists('dblog') &&
          $this->currentUser()->hasPermission('access site reports')
      ) {
      $form['error']['#description'] = $this->t(
      ...
      $form['error']['#description'] = $this->t(
            'If enabled, outdated links in content providing a status <em>Moved Permanently</em> (status code 301) are automatically updated to the most recent URL. If used, it is recommended to use a value of <em>three</em> to make sure this is not only a temporarily change. This feature trust sites to provide a valid permanent redirect. A new content revision is automatically created on link updates if <em>create new revision</em> is enabled in the <a href=":content_types">content types</a> publishing options. It is recommended to create new revisions for all link checker enabled content types. Link updates are nevertheless always logged.',
            [
              ':content_types' => Url::fromRoute(
                    'entity.node_type.collection'
              )->toString(),
    ...
    $form['error']['linkchecker_action_status_code_404'] = [
      ...
      '#description' => $this->t(
              'If enabled, content with one or more broken links (status code 404) will be unpublished and moved to moderation queue for review after the number of specified checks failed. If used, it is recommended to use a value of <em>three</em> to make sure this is not only a temporarily error.'
      ),
      '#type' => 'select',
    ...
    $form['error']['linkchecker_ignore_response_codes'] = [
      ...
      '#description' => $this->t(
              'One HTTP status code per line, e.g. 403.'
      ),
    ];
    ...
    $description .=
            '<p>' .
            $this->t(
              '<strong>Note</strong>: These functions ONLY collect the links, they do not evaluate the HTTP response codes, this will be done during normal cron runs.'
          ) .
            '</p>';
    ...
    $form['clear']['linkchecker_clear_analyze'] = [
      '#type' => 'submit',
      '#value' => $this->t(
              'Clear link data and analyze content for links'
      ),
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $form_state->setValue(
          ...
          trim($form_state->getValue('linkchecker_ignore_response_codes'))
      );
    ...
    foreach ($ignore_response_codes as $ignore_response_code) {
      if (!$this->linkCheckerResponseCodes->isValid($ignore_response_code)) {
        $form_state->setErrorByName(
              'linkchecker_ignore_response_codes',
              $this->t('Invalid response code %code found.', [
    ...
    $linkchecker_disable_link_check_for_urls = array_filter(
          preg_split(
              '/(\r\n?|\n)/',
              $form_state->getValue('linkchecker_disable_link_check_for_urls')
          )
      );
    $linkchecker_reserved_documentation_domains =
            "example.com\nexample.net\nexample.org";
    $form_state->setValue(
          'linkchecker_disable_link_check_for_urls',
          implode(
    ...
    $linkchecker_impersonate_account = user_load_by_name(
          $form_state->getValue('linkchecker_impersonate_account')
      );
    // @todo Cleanup
    // if (empty($linkchecker_impersonate_account->id())) {
    if (
          $linkchecker_impersonate_account &&
          empty($linkchecker_impersonate_account->id())
      ) {
      $form_state->setErrorByName(
            'linkchecker_impersonate_account',
            $this->t('User account %name cannot found.', [
